-- Fast, deterministic queries over loaded world resources. Resource policies
-- belong in callers; this module only discovers positioned items and objects.
...
local function objectKeyFor(object, x, y, z, ordinal)
    local sprite = call(call(object, "getSprite"), "getName")
    local id = call(object, "getID")
...
local function addObject(output, seen, object, square, ordinal, originX,
    originY, originZ)
    ...
    output[#output + 1] = {
        ...
        distSq = dx * dx + dy * dy,
        key = objectKeyFor(object, x, y, z, ordinal),
    }
end

local function scanSquareObjects(output, seen, square, originX, originY,
    originZ)
    local objects = call(square, "getObjects")
    for index = 0, listSize(objects) - 1 do
        addObject(output, seen, listItem(objects, index), square, index,
...
function Locator.FindObject(origin, options)
    ...
    local radius = math.max(0, math.floor(tonumber(options.radius)
        or Locator.DEFAULT_RADIUS))
    local key = cacheKey(originX, originY, originZ, {
    ...
    for dx = -radius, radius do
        for dy = -radius, radius do
            local square = cell:getGridSquare(baseX + dx, baseY + dy, originZ)
            if square then
                scanSquareObjects(output, seen, square, originX, originY,
                    originZ)
            end
        end
    end
...
Locator.ObjectKeyFor = objectKeyFor
