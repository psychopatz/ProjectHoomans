...
local function applyManualActivityDiagnostics(snapshot, state)
    ...
    local diagnostic
    local index
    if type(diagnostics) ~= "table" or type(people) ~= "table" then
        return
    end
    for index = 1, #people do
        person = people[index]
        diagnostic = person and diagnostics[tostring(person.id or "")]
        if diagnostic then person.manualActivityDiagnostic = diagnostic end
    end
end
...
return PNC.ColonyManagementClient
