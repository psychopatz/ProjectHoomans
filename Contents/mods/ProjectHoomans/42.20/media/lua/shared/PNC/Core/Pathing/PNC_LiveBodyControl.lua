...
function LiveBodyControl.IsGrounded(zombie)
    ...
    if GROUNDED_STATES[actionState] == true
...
local function resolveGroundAttacker(record, zombie)
...
local function isFriendlyGroundAttacker(record, attacker)
    local other
    ...
    return tostring(record.faction or "")
        == tostring(other.faction or "")
end

local function counterStagger(record, zombie, attacker)
    ...
    if isFriendlyGroundAttacker(record, attacker) then
        return false
    end
...
function LiveBodyControl.TickGroundedRecovery(record, zombie, now)
    ...
    attacker = resolveGroundAttacker(record, zombie)
    if attacker and isFriendlyGroundAttacker(record, attacker) then
    ...
    if attacker and state.counterAttempted ~= true then
        ...
        if randomUnit() < (
            tonumber(
                PNC.Const
                    and PNC.Const.NPC_GROUNDED_COUNTER_STAGGER_CHANCE
            ) or 0.40
        ) then
            state.counterSucceeded = counterStagger(record, zombie, attacker)
        end
    end
