...
PNC.PlayerCharacters = {
    ...
    EnsureIdentity = function(_, context)
        ...
        T.equal(context.callback, "knowledge_disclosure",
...
PNC.PlayerContext = {
    Resolve = function(_, reason)
        if not characterUUID then
            identityEnsureCalls = identityEnsureCalls + 1
            T.equal(reason, "knowledge_disclosure",
                "disclosure owns identity recovery callback")
            characterUUID = "char_a"
        end
        return {
...
T.truthy(introduction and introduction.revealed[1] == "identity.name",
    "stranger introduction reveals identity name")
T.equal(identityEnsureCalls, 1,
    "disclosure recovers identity when lifecycle has not bound it")
T.truthy(identitySaveCalls >= 1,
...
T.truthy(lifelong and #lifelong.revealed >= 13,
    "lifelong companion initializes every available dossier fact")
T.equal(Knowledge.GetDescriptor(
...
T.truthy(PNC.KnowledgeProviders.Register("test_provider", { GetValue = function() return "blue" end }), "provider registration")
T.truthy(PNC.KnowledgeResolvers.Register("test_direct", { Resolve = function(_, evidence) return evidence[1] and { value = evidence[1].payload.observedValue, confidence = 1, status = "confirmed" } or nil end }), "resolver registration")
T.truthy(PNC.KnowledgeDescriptors.Register({ id = "test.favorite_color", category = "misc", providerID = "test_provider", resolverID = "test_direct", valueType = "categorical", privacy = "personal", discovery = { allowDisclosure = true } }), "descriptor registration")
T.equal(PNC.KnowledgeDescriptors.Register({ id = "test.favorite_color", category = "misc", providerID = "test_provider", resolverID = "test_direct", valueType = "categorical", privacy = "personal" }), false, "duplicate descriptor rejected")

local forced = Knowledge.ForceRevealForPlayer({}, npc.id, "test.favorite_color", 10)
T.truthy(forced ~= nil, "fake descriptor reveals through provider")
T.equal(Knowledge.GetDescriptor("char_a", npc.id, "test.favorite_color").value, "blue", "fake descriptor resolved")

-- Private orientation cannot be inferred by observations; it becomes known only by disclosure.
T.equal(Knowledge.RecordEvidence({ characterUUID = "char_a", npcID = npc.id, descriptorID = "personality.orientation", sourceType = "observed_behavior", strength = 1 }), nil, "private descriptor rejects observation")
local disclosed = Knowledge.Disclose("char_a", npc.id, "personality.orientation", { worldAgeHours = 20 })
T.truthy(disclosed ~= nil, "orientation disclosure")
T.equal(Knowledge.GetDescriptor("char_a", npc.id, "personality.orientation").value, "gay", "orientation truth only copied on disclosure")
...
T.truthy(Knowledge.GetDescriptor("char_a", npc.id, "personality.compassion") ~= nil, "personality inferred")
local normal = Knowledge.BuildPlayerSnapshot("char_a", npc.id)
safe(normal)
T.equal(normal.truth, nil, "normal snapshot contains no truth")
-- Presentation consumes generic descriptor rows; no descriptor-specific UI code.
T.load(T.path("ProjectHoomans", "client", "PNC/UI/Knowledge/PNC_KnowledgePresentation.lua"))
local dossierRows = PNC.KnowledgePresentation.BuildDossierRows(normal)
T.truthy(#dossierRows > 0, "generic dossier model displays discovered descriptors")
...
T.equal(#dossierWithoutSkills, 1,
    "dossier omits capabilities already represented by Skills tab")
T.equal(dossierWithoutSkills[1].id, "identity",
...
T.equal(#learned, 1, "new dossier facts produce visual-feedback candidates")
T.equal(PNC.KnowledgePresentation.GetFactLabel(learned[1]), "Name",
...
T.equal(PNC.KnowledgePresentation.ShowLearnedFacts(
    { categories = {} },
    { categories = { {
        id = "identity",
        descriptors = { { descriptorID = "identity.name" } },
...
T.truthy(found, "debug snapshot is descriptor driven and can show truth")
T.truthy(#PNC.KnowledgePresentation.BuildDebugRows(debug, { showTruth = true }) >= 13,
    "generic debug model displays registered descriptors")
for _, row in ipairs(debug.rows) do
    T.equal(row.evidence, nil, "debug summary defers raw evidence")
end
...
T.truthy(type(detailedEvidence) == "table", "debug detail request includes selected evidence only")
...
T.truthy(debugAction and debugAction.actionResult ~= nil,
...
T.truthy(topicDiscovery and topicDiscovery.actionResult and #topicDiscovery.actionResult.revealed == 10,
    "debug discovery reveals only the selected conversational topic")
T.equal(Knowledge.Dirty, false,
...
T.equal(debugCompassion.status, "confirmed", "debug discovery confirms numeric trait bands")
T.equal(debugCompassion.value, "high", "debug discovery resolves numeric trait direction")
for index = 1, 20 do
...
T.equal(loyaltyEvidence, 16, "evidence per descriptor limit")
...
T.truthy(normalized.byCharacter.char_a.byNPC.npc_russell.discovered["removed.mod.fact"] ~= nil, "orphaned fact preserved")

-- New characters remain isolated; no all-character matrix is created.
T.equal(Knowledge.Get("char_b", npc.id), nil, "new survivor has no inherited notes")
...
T.equal(learned, 2000, "sparse scale evidence count")
T.equal(#PNC.KnowledgeDescriptors.List(), 67, "scale descriptors registered without schema migration")
...
T.truthy(Knowledge.Save(), "knowledge writes through the normal world-save path")
Knowledge.Registry = { schemaVersion = 1, revision = 0, byCharacter = {} }
Knowledge.Loaded = false
Knowledge.Dirty = false
T.truthy(Knowledge.Load(), "knowledge reloads from world ModData")
T.equal(Knowledge.GetDescriptor("char_a", npc.id, "identity.name").value,
    "Burton Gilmore", "introduced identity survives reload")
T.equal(Knowledge.GetDescriptor("char_a", npc.id, "test.favorite_color").value,
    "blue", "learned fact survives reload")
local restored = Knowledge.BuildKnownSnapshotsForPlayer({})
T.truthy(#restored > 0, "known snapshots hydrate a reconnecting client")
safe(Knowledge.Registry)
T.finish("pnc_knowledge_smoke")
