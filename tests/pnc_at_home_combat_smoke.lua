local T = require "tests/support/test"

local FILE = T.path(
    "ProjectHoomans",
    "shared",
    "PNC/Core/Behaviors/PNC_Behavior_AtHome.lua"
)
local STATIC_FILE = T.path(
    "ProjectHoomans",
    "shared",
    "PNC/Core/Behaviors/BehaviorCompanion/PNC_BehaviorCompanion_StaticOrders.lua"
)

local cleared = 0
local halted = 0
local applied = 0

PNC = {
    Const = { GUARD_ENGAGE_RADIUS = 4 },
    Core = {
        Distance = function(x, y, targetX, targetY)
            local dx = (tonumber(x) or 0) - (tonumber(targetX) or 0)
            local dy = (tonumber(y) or 0) - (tonumber(targetY) or 0)
            return math.sqrt(dx * dx + dy * dy)
        end,
    },
    BehaviorCommon = {
        ClearCombatTarget = function(record)
            cleared = cleared + 1
            record.runtime.inCombatUntil = 0
        end,
        HaltMovement = function() halted = halted + 1 end,
        MoveRecord = function() end,
    },
    Animation = { Apply = function() applied = applied + 1 end },
    NavigationRouter = { Clear = function() end },
    OrderSystem = { RegisterNormalizer = function() end },
    JobSystem = { RegisterOrder = function() end },
    BehaviorRegistry = { Register = function() end },
}

T.load(FILE)

PNC.BehaviorCompanion = { Internal = {} }
T.load(STATIC_FILE)

local record = {
    id = "resident",
    x = 10,
    y = 10,
    z = 0,
    runtime = { inCombatUntil = 999999 },
    orderSpec = {
        kind = "colony_home",
        baseId = "base-1",
        x = 10,
        y = 10,
        z = 0,
        radius = 2,
    },
}

T.truthy(PNC.BehaviorAtHome.Tick(record, {}),
    "At Home resumes its idle behavior")
T.equal(record.activeBehavior, "AtHome",
    "At Home keeps passive ownership when ThreatGuard is not running")
T.equal(cleared, 1,
    "At Home clears stale combat state while idling")

T.truthy(PNC.BehaviorAtHome.Tick(record, {}),
    "At Home resumes after its threat disappears")
T.equal(record.activeBehavior, "AtHome",
    "At Home returns to idle behavior after combat")
T.equal(cleared, 2,
    "At Home clears the stale combat lease after combat")
T.equal(halted, 2, "At Home resumes its idle movement hold")
T.equal(applied, 2, "At Home resumes its idle animation")
T.equal(record.orderSpec.kind, "colony_home",
    "temporary combat does not replace the durable home order")

local stayRecord = {
    id = "guard",
    x = 10,
    y = 10,
    z = 0,
    anchorX = 10,
    anchorY = 10,
    anchorZ = 0,
    orderSpec = { kind = "guard", x = 10, y = 10, z = 0 },
    runtime = {},
}
T.truthy(PNC.BehaviorCompanion.Internal.TickGuardAnchor(stayRecord, {}),
    "GuardAnchor advances its passive behavior")
T.equal(stayRecord.activeBehavior, "GuardAnchor",
    "GuardAnchor keeps passive ownership when ThreatGuard is not running")
T.truthy(PNC.BehaviorCompanion.Internal.TickGuardAnchor(stayRecord, {}),
    "Stay resumes after its threat disappears")
T.equal(stayRecord.activeBehavior, "GuardAnchor",
    "Stay returns to its idle behavior after combat")

-- GuardAnchor must not reissue its anchor route if a seat presentation still
-- owns the body during an order/scene handoff.
local guardMoves = 0
local guardHolds = 0
PNC.BehaviorCommon.MoveRecord = function()
    guardMoves = guardMoves + 1
end
PNC.BehaviorCommon.HaltMovement = function()
    guardHolds = guardHolds + 1
end
PNC.LiveBodyControl = {
    IsSeated = function() return true end,
    IsPresentationCombatActive = function() return false end,
}
stayRecord.x = 20
T.truthy(PNC.BehaviorCompanion.Internal.TickGuardAnchor(stayRecord, {}),
    "seated guard stays handled by GuardAnchor")
T.equal(stayRecord.activeBehavior, "GuardAnchor:seated",
    "seated guard exposes its presentation hold")
T.equal(guardMoves, 0,
    "seated guard reissued movement to its anchor")
T.equal(guardHolds, 1,
    "seated guard did not hold its movement lane")

T.finish("pnc_at_home_combat_smoke")

T.finish("pnc_at_home_combat_smoke")
