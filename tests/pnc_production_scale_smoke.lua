...
for id = 1, 5000 do
    T.equal(Registry.Commands.GetOrCreateId("ScaleMod.Recipe" .. tostring(id)),
        id, "monotonic id")
end
T.equal(Registry.Queries.Diagnostics().persistentRecipeIdCount, 5000,
...
for _, state in pairs(Repository.ByColony) do
    ...
    T.equal(state.learnedRecipeSet, nil, "runtime set not persisted on state")
    T.truthy(Repository.Runtime[state.colonyId].learnedRecipeSet[2000] == true,
        "runtime O(1) membership missing")
end
T.equal(learnedCount, 100000, "compact learned ids")
T.equal(Registry.Queries.Diagnostics().persistentRecipeIdCount, 5000,
...
T.truthy(approximateBytes < 1000000,
    "dense numeric knowledge proxy unexpectedly large")
T.finish("pnc_production_scale_smoke")
