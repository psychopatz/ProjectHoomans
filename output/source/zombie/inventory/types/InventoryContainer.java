...
public final class InventoryContainer extends InventoryItem {
    ...
    public void setCapacity(int capacity) {
        ...
        if (capacity > 50.0F - this.actualWeight) {
            DebugType.General.warn("Attempting to set capacity of " + this + "over maximum capacity of " + (50.0F - this.actualWeight));
        }
    ...
    public void DoTooltip(ObjectTooltip tooltipUI) {
        ...
        if (!this.getItemContainer().getItems().isEmpty()) {
            ...
            for (int i = this.getItemContainer().getItems().size() - 1; i >= 0; i--) {
                InventoryItem item = (InventoryItem)this.getItemContainer().getItems().get(i);
    ...
    public void DoTooltip(ObjectTooltip tooltipUI, Layout layout) {
        ...
        if (this.getEffectiveCapacity(tooltipUI.getCharacter()) != 0) {
            ...
            item.setLabel(Translator.getText("Tooltip_container_Capacity", new Object[0]) + ":", 1.0F, 1.0F, 1.0F, 1.0F);
            item.setValueRightNoPlus(this.getEffectiveCapacity(tooltipUI.getCharacter()));
        }

        if (this.getWeightReduction() != 0) {
            LayoutItem item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_container_Weight_Reduction", new Object[0]) + ":", 1.0F, 1.0F, 1.0F, 1.0F);
            item.setValueRightNoPlus(this.getWeightReduction());
        }

        if (this.getMaxItemSize() != 0.0F) {
            LayoutItem item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_container_Max_Item_Size", new Object[0]) + ":", 1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public float getDirtiness() {
        List<BloodBodyPartType> coveredParts = BloodClothingType.getCoveredParts(this.getBloodClothingType());
        float totalDirt = 0.0F;

        for (int i = 0; i < coveredParts.size(); i++) {
            totalDirt += this.getDirt(coveredParts.get(i));
        }
    ...
    public float getContentsWeight() {
        float weight = this.getInventory().getContentsWeight();
        if (this.getFluidContainer() != null) {
            weight += this.getFluidContainer().getAmount();
        }
