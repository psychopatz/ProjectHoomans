...
public class Moveable extends InventoryItem {
    ...
    public String getName(IsoPlayer player) {
        if (this instanceof Radio && ((Radio)this).getDeviceData() != null && ((Radio)this).getDeviceData().getIsTurnedOn()) {
            return Translator.getText("IGUI_ClothingNaming", new Object[]{Translator.getText("Tooltip_activated", new Object[0]), this.name});
    ...
    public boolean ReadFromWorldSprite(String sprite) {
        ...
        try {
            IsoSprite spr = (IsoSprite)IsoSpriteManager.instance.namedMap.get(sprite);
            if (spr != null) {
                PropertyContainer props = spr.getProperties();
                if (props.has("IsMoveAble")) {
                    ...
                    if (props.has(IsoPropertyType.CUSTOM_NAME)) {
                        if (props.has(IsoPropertyType.GROUP_NAME)) {
                            customName = props.get(IsoPropertyType.GROUP_NAME) + " " + props.get(IsoPropertyType.CUSTOM_NAME);
                        } else {
                            customName = props.get(IsoPropertyType.CUSTOM_NAME);
                        }
                    }
            ...
            DebugType.Moveable.printException(e, LogSeverity.Error, "Error in Moveable item: %s", new Object[]{e.getMessage()});
        }
    ...
    public void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        if (!this.ReadFromWorldSprite(this.worldSprite)
            ...
            DebugType.Moveable.debugln("Moveable.load -> Radio item = %s", new Object[]{this.fullType != null ? this.fullType : "unknown"});
        }
        ...
        this.isLight = input.get() != 0;
        if (this.isLight) {
            this.lightUseBattery = input.get() != 0;
            this.lightHasBattery = input.get() != 0;
            if (input.get() != 0) {
