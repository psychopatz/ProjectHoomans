...
public final class WeaponPart extends InventoryItem implements Drainable, IUpdater {
    ...
    public void DoTooltip(ObjectTooltip tooltipUI, Layout layout) {
        ...
        item.setLabel(Translator.getText("Tooltip_weapon_Type", new Object[0]) + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
        item.setValue(Translator.getText("Tooltip_weapon_" + this.partType, new Object[0]), 1.0F, 1.0F, 0.8F, 1.0F);
        item = layout.addItem();
        String mountTxt = Translator.getText("Tooltip_weapon_CanBeMountOn", new Object[0])
            + ": "
    ...
    public void DoBatteryTooltip(ObjectTooltip tooltipUI, Layout layout) {
        if (this.hasTag(ItemTag.USES_BATTERY) && !this.hasTag(ItemTag.HIDE_REMAINING)) {
            LayoutItem item = layout.addItem();
            item.setLabel(Translator.getText("IGUI_invpanel_Remaining", new Object[0]) + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
    ...
    public boolean canAttach(IsoGameCharacter character, HandWeapon weapon) {
        ...
        if (!this.mountOn.isEmpty() && !this.mountOn.contains(weapon.getFullType())) {
            return false;
        }

        if (StringUtils.isNullOrEmpty(this.canAttachCallback)) {
            return true;
        }
