...
public final class AlarmClock extends InventoryItem implements IAlarmClock {
    ...
    public AlarmClock(String module, String name, String itemType, String texName) {
        ...
        this.itemType = ItemType.ALARM_CLOCK;
        this.randomizeAlarm();
    }
    ...
    public void update() {
        if (this.alarmSet) {
            ...
            if (this.isRinging()) {
                ...
                this.ringSince = GameTime.minHours(this.ringSince, worldAgeHours);
    ...
    public void DoTooltip(ObjectTooltip tooltipUI, Layout layout) {
        ...
        item.setLabel(Translator.getText("IGUI_CurrentTime", new Object[0]), 1.0F, 1.0F, 0.8F, 1.0F);
        ...
        if (this.alarmSet) {
            item = layout.addItem();
            item.setLabel(Translator.getText("IGUI_AlarmIsSetFor", new Object[0]), 1.0F, 1.0F, 0.8F, 1.0F);
    ...
    public void syncAlarmClock_Player(IsoPlayer player) {
        if (GameClient.client) {
            INetworkPacket.send(
                PacketType.SyncPlayerAlarmClock, new Object[]{player, this.id, false, this.alarmHour, this.alarmMinutes, this.alarmSet, this.forceDontRing}
            );
        } else if (GameServer.server) {
            IsoPlayer ownerPlayer = this.getOwnerPlayer(this.container);
            if (ownerPlayer != null) {
                INetworkPacket.sendToRelative(
                    PacketType.SyncPlayerAlarmClock,
                    ownerPlayer.getSquare().getX(),
                    ownerPlayer.getSquare().getY(),
                    new Object[]{player, this.id, false, this.alarmHour, this.alarmMinutes, this.alarmSet, this.forceDontRing}
                );
            }
        }
    }
    ...
    public void syncAlarmClock_World() {
        if (GameClient.client) {
            INetworkPacket.send(
                PacketType.SyncWorldAlarmClock,
                new Object[]{
    ...
    public void syncStopRinging() {
        ...
        if (player != null) {
            INetworkPacket.sendToRelative(
                PacketType.SyncPlayerAlarmClock, player.getX(), player.getY(), new Object[]{player, this.id, true, 0, 0, false, this.forceDontRing}
            );
    ...
    public void stopRinging() {
        ...
        if (this.ringSound != -1L) {
            ...
            IsoGridSquare sq = this.getAlarmSquare();
            if (sq != null) {
                BaseSoundEmitter emitter = IsoWorld.instance.getFreeEmitter(sq.x + 0.5F, sq.y + 0.5F, sq.z);
                emitter.playSoundImpl("AlarmClockRingingStop", sq);
            }
        }
