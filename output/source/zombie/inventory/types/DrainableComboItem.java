...
public final class DrainableComboItem extends InventoryItem implements Drainable, IUpdater {
    ...
    private static final float TOWEL_WET_TIME_MULTIPLIER = 0.5F;
    ...
    public boolean shouldUpdateInWorld() {
        return !GameServer.server && this.heat != 1.0F || this.canBecomeWetInRain();
    }
    ...
    private boolean canBecomeWetInRain() {
        ...
        return worldItem != null && worldItem.getObjectIndex() != -1 && worldItem.getSquare().isOutside()
            ? this.is(new ItemKey[]{zombie.scripting.objects.ItemKey.Drainable.BATH_TOWEL, zombie.scripting.objects.ItemKey.Drainable.DISH_CLOTH})
            : false;
    }

    private void replaceWetTowel() {
        InventoryItem item = InventoryItemFactory.CreateItem(this.getReplaceOnDeplete());
        this.getWorldItem().swapItem(item);
    }

    private void updateTowelWetness() {
        if (RainManager.isRaining() && this.canBecomeWetInRain()) {
            int uses = this.getCurrentUses();
            if (uses <= 0) {
                this.replaceWetTowel();
            } else {
                this.towelWetnessAccumulator = this.towelWetnessAccumulator + 0.5F * RainManager.getRainIntensity() / 1000.0F * this.timeMultiplier;
                int use = (int)(this.towelWetnessAccumulator / this.getUseDelta());
                if (use > 0) {
                    uses = Math.max(uses - use, 0);
                    this.setCurrentUses(uses);
                    this.syncItemFields();
                    this.towelWetnessAccumulator = this.towelWetnessAccumulator - use * this.getUseDelta();
                    if (uses <= 0) {
                        this.replaceWetTowel();
                    }
                }
            ...
            this.towelWetnessAccumulator = 0.0F;
        }
    }
    ...
    public void update() {
        ...
        if (outermostContainer != null) {
            ...
            if (this.heat > temp) {
                this.heat = this.heat - 0.001F * this.timeMultiplier;
            ...
            if (this.heat < temp) {
                this.heat = this.heat + temp / 1000.0F * this.timeMultiplier;
            ...
            if (p == null || (!this.canBeActivated() || !this.isActivated()) && this.canBeActivated() || !p.isHandItem(this) && !p.isAttachedItem(this)) {
                ...
                if (minutes != this.lastUpdateMinutes) {
                    if (this.lastUpdateMinutes > -1) {
                        this.Use(false, false, GameClient.client);
                    }
        ...
        if (this.useWhileUnequiped && this.uses > 0 && (this.canBeActivated() && this.isActivated() || !this.canBeActivated())) {
            this.ticks = this.ticks + this.timeMultiplier;
        ...
        if (!GameClient.client && this.getCurrentUses() <= 0 && this.getReplaceOnDeplete() != null) {
    ...
    public void syncItemFields() {
        ...
        if (outer != null && outer.getParent() instanceof IsoPlayer) {
            if (GameClient.client) {
                ...
                INetworkPacket.send((IsoPlayer)outer.getParent(), PacketType.ItemStats, new Object[]{this.getContainer(), this});
            }
        }
    }
    ...
    public boolean finishupdate() {
        ...
        return !this.canBecomeWetInRain();
    }
