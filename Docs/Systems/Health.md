...
- fatal infection bypasses ordinary incapacitation and creates an inert corpse; after `NPCReanimationHours`, only the host/server calls the vanilla corpse-reanimation routine, which preserves the NPC's appearance and carried/worn items while producing one ordinary, vulnerable zombie
